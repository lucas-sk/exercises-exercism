...
export function age(planet: string, seconds: number): number {
	...
	const findIndexPlanet = listPlanetAndPeriodRelationEarth.findIndex(
		(InfoItemPlanet) => InfoItemPlanet.planet === planet.toLocaleLowerCase(),
	);

	const periodPlanetRelationEarth =
		listPlanetAndPeriodRelationEarth[findIndexPlanet].seconds;

	const calcAge = (
		seconds /
		(periodPlanetRelationEarth * oneYearOfEarthInSeconds)
	).toFixed(2);
