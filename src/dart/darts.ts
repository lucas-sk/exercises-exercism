...
export function score(x: number, y: number): unknown {
  const valueOfXPow = Math.pow(x, 2)
  const valueOfyYPow = Math.pow(y,  2)
  const distanceInBetweenTwoPoints = Math.sqrt(valueOfXPow + valueOfyYPow)

  const isInsideCircleIndex = listCircle.findIndex((item) => distanceInBetweenTwoPoints <= item.valueRadius)
